...
parser.add_argument('--multiDemand', '-mD', help='数据包大小', default=6.0, type=float)
...
maddpg_agents = MADDPG(actor_dims, critic_dims, n_agent, n_action_c, n_action_p, 0.1, 0.1, 0.1, 0.1, device='cpu', actor_mutli=actor_muti, critic_mutli=critic_muti)
for i in range(env.n_V2V):
    i_agent = i
...
for i_ep in range(test_episode):
    ...
    print('         Random            DQN         DDPG')
    print(
        'V2I Rate: {:.3f}               {:.3f}        {:.3f}'.format(np.average(np.asarray(sum_V2I_rand)),
                                                                       np.average(np.asarray(sum_V2I_dqn)),
                                                                       np.average(np.asarray(sum_V2I_ddpg))))
    print('SecTrans: {:.3f}               {:.3f}        {:.3f}'.format(np.average(np.asarray(SecTrans_Pro_rand)),
...
with open(data_path, 'w', newline='') as file:
    ...
    writer.writerow(['', 'Random', 'DQN', 'DDPG'])
